  ...
  public class EasyColliderWindow : EditorWindow
  {
...
#else
      if (ECEditor.SelectedGameObject != null && stage != _currentPrefabStage)
      {
        // entering a prefab stage with a selected gameobject should null out the object regardless so people don't think they can't select the prefab object.
    ...
    private void DrawAutoSkinnedMeshTools()
    {
      ...
      if (ECUI.DisableableButton("Generate Colliders on Skinned Mesh", "Creates colliders along the chain of a skinned mesh collider's bones.", "No skinned mesh found on the selected gameobject or it's children", ECEditor.HasSkinnedMeshRenderer || ECAutoSkinned.renderer != null))
      {
        ...
        {
          //because this shouldn't be used, but the methods used are the same so it will get them from there anyway.
#if(UNITY_EDITOR)
          ECEPreferences.ShrinkGrow = 1f;
#endif
    ...
    private void DrawColliderCreationMethods()
    {
      ...
      if (CurrentTab != ECE_WINDOW_TAB.Editing)
      {
        ...
        if (ECEPreferences.VertexNormalOffsetType != NORMAL_OFFSET.In)
        {
          ...
          if (ECUI.IconButton("d_Refresh", "Reset value to 0.0f"))
          {
        ...
        SetLabelWidth(100f);
        EditorGUI.BeginChangeCheck();
        float shrinkGrow = EditorGUILayout.FloatField(new GUIContent("Shrink/Grow:", "Shrink or grow the resulting collider by this value, similar to scaling an object.\n\n Useful for when you want to match the exact shape, but want it to be slightly larger or smaller for gameplay reasons."), ECEPreferences.ShrinkGrow, GUILayout.MinWidth(40f));
        if (EditorGUI.EndChangeCheck())
        {
          Undo.RecordObject(ECEPreferences, "Change Shrink Grow value");
          ECEPreferences.ShrinkGrow = shrinkGrow;
          UpdatePreview(true);
        }
        if (ECUI.IconButton("d_Refresh", "Reset value to 1.0f"))
        {
          Undo.RecordObject(ECEPreferences, "Reset shrink grow value to 1");
          ECEPreferences.ShrinkGrow = 1.0f;
          UpdatePreview(true);
        }
        RestoreLabelWidth();
      ...
      // options for each specific collider type go below the general options.
      if (ECEPreferences.PreviewColliderType == CREATE_COLLIDER_TYPE.SPHERE)
      {
      ...
      else if (ECEPreferences.PreviewColliderType == CREATE_COLLIDER_TYPE.CAPSULE || ECEPreferences.PreviewColliderType == CREATE_COLLIDER_TYPE.ROTATED_CAPSULE)
      {
      ...
      else if (ECEPreferences.PreviewColliderType == CREATE_COLLIDER_TYPE.CONVEX_MESH)
      {
      ...
      else if (ECEPreferences.PreviewColliderType == CREATE_COLLIDER_TYPE.CYLINDER)
      {
        SetLabelWidth(125f);
        EditorGUI.BeginChangeCheck();
