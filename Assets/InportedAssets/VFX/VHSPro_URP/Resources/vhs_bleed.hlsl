...
half3 t2d(float2 p){
   half3 col = SAMPLE_TEXTURE2D_X(_InputTex, sampler_InputTex, p ).xyz; //output color
   return rgb2yiq( col );
}
...
half3 vhs2( Varyings input ){
   ...
   float2 p = input.uv; // normalized tex coordnates 0..1  
   float t = _time;          
   ...
   if(feedbackOn==1){
      half3 fbb = SAMPLE_TEXTURE2D_X(_FeedbackTex, sampler_FeedbackTex, p ).xyz; //output color
      col = bms(col, fbb*1.);       
   }
