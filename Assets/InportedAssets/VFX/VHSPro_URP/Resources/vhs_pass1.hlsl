...
float3 jitterV(float2 p, float m, float t){
   ...
   half3 signal = half3(
      ...
      rgb2yiq( SAMPLE_TEXTURE2D_X(_InputTex, sampler_InputTex, float2(offx.r, p.y) ).rgb ).x,
      rgb2yiq( SAMPLE_TEXTURE2D_X(_InputTex, sampler_InputTex, float2(offx.g, p.y) ).rgb ).y,
      rgb2yiq( SAMPLE_TEXTURE2D_X(_InputTex, sampler_InputTex, float2(offx.b, p.y) ).rgb ).z
   );
...
float2 jitterH(float2 p, float t){
   ...
   float2 tapeNoiseUV(float2 p, float2 pn){
      ...
      for (int ii = 0; ii < distWidth % 1023; ii++){
        ...
        float tnl = SAMPLE_TEXTURE2D_X(_TapeTex, sampler_TapeTex, float2(0., pn.y-_OneN.x*ii) ).y;
   ...
   half3 tapeNoiseCol(half3 signal, half4 colN, float2 pn){
      ...
      for(int j=0; j<tailLength % 1023; j++){
         ...
         float4 colN2 = SAMPLE_TEXTURE2D_X(_TapeTex, sampler_TapeTex, d ).xyzw; 
...
half3 vhs( Varyings input ){
   ...
   half3 signal = half3(0,0,0);  //signal has negative values
      ...
      col = SAMPLE_TEXTURE2D_X(_InputTex, sampler_InputTex, p ).xyz; 
      signal = rgb2yiq(col);
   ...
   half4 colN = half4(0,0,0,0);
      ...
      colN = SAMPLE_TEXTURE2D_X(_TapeTex, sampler_TapeTex, pn ).xyzw;      
   #endif
