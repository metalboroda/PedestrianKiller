...
namespace ECE
{
  ...
  public class EasyColliderEditor : ScriptableObject, ISerializationCallbackReceiver
  {
    ...
    [SerializeField]
    private List<EntityId> _AddedColliderIDs;
    /// <summary>
    /// List of added colliders through GetInstanceID()
    /// </summary>
    public List<EntityId> AddedColliderIDs
    {
      get
      {
        if (_AddedColliderIDs == null)
        {
          _AddedColliderIDs = new List<EntityId>();
        }
    ...
    [SerializeField]
    private List<EntityId> _AddedInstanceIDs;
    /// <summary>
    ...
    /// </summary>
    public List<EntityId> AddedInstanceIDs
    {
      get
      {
        if (_AddedInstanceIDs == null)
        {
          _AddedInstanceIDs = new List<EntityId>();
        }
    ...
    public void ChangeRenderPointType(RENDER_POINT_TYPE value)
    {
      ...
      if (value == RENDER_POINT_TYPE.GIZMOS && Gizmos == null && SelectedGameObject != null)
      {
        ...
        AddedInstanceIDs.Add(GetIDFor(Gizmos));
      }
      if (value == RENDER_POINT_TYPE.SHADER && Compute == null && SelectedGameObject != null)
      {
        ...
        AddedInstanceIDs.Add(GetIDFor(Compute));
      }
    }
    ...
    {
      // var so it works with 6000_3 and 6000_4 or newer (int to entity id switch)
      foreach (var id in AddedInstanceIDs)
      {
      ...
      if (go != null)
      {
        ...
        {
          if (AddedColliderIDs.Contains(GetIDFor(col)))
          {
            col.enabled = true;
          }
        }
    ...
    {
#if UNITY_6000_4_OR_NEWER
      AddedInstanceIDs = new List<EntityId>();
#else
    ...
    public void DisableCreatedCollider(Collider col)
    {
      ...
      AddedColliderIDs.Add(GetIDFor(col));
    }
...
#if (UNITY_6000_4_OR_NEWER)
    /// <summary>
    /// method to get id for any unityengine.object
    /// </summary>
    /// <param name="obj"></param>
    /// <returns>entity id or instance id depending on version of unity.</returns>
    EntityId GetIDFor(Object obj)
    {
      return obj.GetEntityId();
    }
    /// <summary>
    /// gets the object an id (entity or instance id depending on unity version) represents
    /// </summary>
    /// <param name="id"></param>
    /// <returns></returns>
    Object GetObjectForID(EntityId id)
    {
    ...
    public Collider[] GetConvertibleColliders()
    {
      ...
      {
        if (AddedInstanceIDs.Contains(GetIDFor(colliders[i])))
        {
          colliders[i] = null;
        }
      }
    ...
    public bool IsSelectableCollider(Collider collider)
    {
      ...
      if (GetAllColliders().Contains(collider))
      {
        ...
        if (AddedInstanceIDs.Contains(GetIDFor(collider))) return false;
        return true;
      }
      return false;
    }
    ...
    public void RemoveSelectedColliders()
    {
      foreach (Collider col in SelectedColliders)
      {
        ...
        if (col == null || AddedInstanceIDs.Contains(GetIDFor(col))) continue;
    ...
    public void InvertSelection()
    {
      ...
      set.RemoveWhere(x => x.enabled == false || AddedInstanceIDs.Contains(GetIDFor(x)));
    ...
    void SetRequiredComponentsFrom(GameObject parent, List<MeshFilter> meshFilters)
    {
      ...
      foreach (MeshFilter filter in meshFilters)
      {
        if (filter != null)
        {
          ...
          AddedInstanceIDs.Add(GetIDFor(collider));
    ...
    MeshFilter SetupFilterForSkinnedMesh(SkinnedMeshRenderer smr)
    {
      ...
      if (filter == null)
      {
        ...
        AddedInstanceIDs.Add(GetIDFor(filterHolder));
      ...
      if (filter != null)
      {
        ...
        AddedInstanceIDs.Add(GetIDFor(filter));
