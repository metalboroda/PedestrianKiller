...
namespace ECE
{
  ...
  public class EasyColliderCreator
  {
    ...
    /// </summary>
    /// <param name="collider_type"></param>
    ...
    /// <param name="shrinkGrow"> adjusts the size of the box collider</param>
    /// <returns>Data appropriate variables set for a box collider</returns>
    public BoxColliderData CalculateBox(List<Vector3> worldVertices, Transform attachTo, bool isRotated = false, bool requiresMatrixCalc = false, float shrinkGrow = 1f)
    {
    ...
    /// <param name="requiresMatrixCalc"> true when the attach to transform is not already correctly rotated for a rotated collider</param>
    /// <param name="shrinkGrow">adjust the height/radius of the capsule collider</param>
    /// <returns>Data with appropriate variables set for a capsule collider</returns>
    public CapsuleColliderData CalculateCapsuleBestFit(List<Vector3> worldVertices, Transform attachTo, bool isRotated, bool requiresMatrixCalc = false, float shrinkGrow = 1f)
    {
    ...
    /// <param name="shrinkGrow">adjust the height/radius of the capsule collider</param>
    /// <returns>Data with appropriate variables set for a capsule collider</returns>
    public CapsuleColliderData CalculateCapsuleMinMax(List<Vector3> worldVertices, Transform attachTo, CAPSULE_COLLIDER_METHOD method, bool isRotated, bool requiresMatrixCalc = false, float shrinkGrow = 1f)
    {
      if (isRotated && worldVertices.Count < 3)
      {
        return new CapsuleColliderData();
    ...
    public MeshColliderData CalculateCylinderCollider(List<Vector3> worldVertices, Transform attachTo, int numberOfSides = 12, CYLINDER_ORIENTATION orientation = CYLINDER_ORIENTATION.Automatic, float cylinderOffset = 0.0f, float shrinkGrow = 1f)
    {
    ...
    /// <param name="shrinkGrow">adjusts the radius of the sphere collider</param>
    /// <returns>Data with appropriate variables set for a sphere collider</returns>
    public SphereColliderData CalculateSphereBestFit(List<Vector3> worldVertices, Transform attachTo, float shrinkGrow = 1f)
    {
    ...
    /// <param name="shrinkGrow">adjusts the radius of the sphere collider</param>
    /// <returns>Data with appropriate variables set for a sphere collider</returns>
    public SphereColliderData CalculateSphereDistance(List<Vector3> worldVertices, Transform attachTo, float shrinkGrow = 1f)
    {
      if (worldVertices.Count < 2)
      {
        return new SphereColliderData();
    ...
    /// <param name="shrinkGrow">adjusts the radius of the sphere collider</param>
    /// <returns>Data with appropriate variables set for a sphere collider</returns>
    public SphereColliderData CalculateSphereMinMax(List<Vector3> worldVertices, Transform attachTo, float shrinkGrow = 1f)
    {
      if (worldVertices.Count < 2)
      {
        return new SphereColliderData();
    ...
    /// <param name="shrinkGrow">shrinks/grows the resulting mesh collider, similar to scaling an object</param>
    /// <returns>The created mesh</returns>
    public Mesh CreateMesh_Messy(List<Vector3> worldSpaceVertices, GameObject attachTo, GameObject selected, float shrinkGrow = 1.0f)
    {
    ...
    /// <param name="attachTo">transform the collider will be attached to, used to convert to local space</param>
    /// <param name="shrinkGrow">shrinks/grows the resulting mesh collider, similar to scaling an object</param>
    /// <returns>EasyColliderData with mesh, matrix, and validity</returns>
    public MeshColliderData CalculateHullData(List<Vector3> points, Transform attachTo, float shrinkGrow = 1.0f)
    {
    ...
    /// <param name="selected">if in the editor, and not-null the output mesh will be saved in a location according to this and preferences, otherwise it will use the attach to.</param>
    /// <param name="shrinkGrow">shrinks/grows the resulting mesh collider, similar to scaling an object</param>
    /// <returns></returns>
    public Mesh CreateMesh_QuickHull(List<Vector3> vertices, GameObject attachTo, bool isLocal = false, GameObject selected = null, float shrinkGrow = 1.0f)
    {
    ...
    /// <param name="shrinkGrow"></param>
    void AdjustVerticesForShrinkGrow(List<Vector3> localVerts, float shrinkGrow)
    {
