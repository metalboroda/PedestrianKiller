...
namespace FIMSpace.AnimationTools
{
    ...
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public void SampleAnimationClip(AnimationClip clip, float time, float humPrevCorr, float humPrevCorrMul)
        {
            if (_anim_MainSet == null) return;

            if ( _anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.SampleAnimationClip)
            {
                clip.SampleAnimation(_anim_MainSet.LatestAnimator.gameObject, time);
            }
            else if ( _anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.Mecanim)
            {
                TargetClip.SampleAnimation(latestAnimator.gameObject, time + humPrevCorr);

                if (currentMecanim == null) return;
                if (currentMecanim.isHuman == false) return;
                if (_anim_MainSet.AnimationSamplingMode != ADClipSettings_Main.EAnimationSamplingMode.Mecanim) return;
                AnimationGenerateUtils.UpdateHumanoidIKPreview(currentMecanim, clip, time * humPrevCorrMul, false);
            }
            else if ( _anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.Playable)
            {
                if (currentMecanim == null) { clip.SampleAnimation(_anim_MainSet.LatestAnimator.gameObject, time); return; }
                UpdatePlayableAnimationPreview(ref playablePreview, currentMecanim, clip, time);
            }
        }

        public void SampleMorphAnimationClip(AnimationClip clip, float time)
        {
            if (_anim_MainSet == null) return;

            if (_anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.Playable)
            {
                if (currentMecanim == null) { clip.SampleAnimation(_anim_MainSet.LatestAnimator.gameObject, time); return; }
                UpdatePlayableAnimationPreview(ref playableMorphPreview, currentMecanim, clip, time);
            }
            else
            {
                SampleAnimationClip(clip, time, 0f, 1f);
            }
        }

        static ADPlayablePreview playablePreview = null;
        static ADPlayablePreview playableMorphPreview = null;
        static ADPlayablePreview playableSecondaryAnimatorPreview = null;

        public static void UpdatePlayableAnimationPreview(ref ADPlayablePreview playable, Animator currentMecanim, AnimationClip clip, float time)
        {
            // Setup playable handler if not exists yet
            if (playable is null) playable = new ADPlayablePreview(currentMecanim);
            else if (playable.characterAnimator != currentMecanim) playable = new ADPlayablePreview(currentMecanim);

            // Update existing playable handler
            playable.SetAnimationClipToPreview(clip);
