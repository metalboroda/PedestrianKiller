                    ...
                    if (additionalSetupSettingsFoldout)
                    {
                        ...
                        if (currentMecanim)
                            if (currentMecanim.isHuman)
                            {
                                ...
                                EditorGUILayout.BeginHorizontal();
                                mainSetup.Additional_AnimationSampling = (ADClipSettings_Main.EAnimationSamplingMode)EditorGUILayout.EnumPopup(new GUIContent("Animation Sampling Mode: ", "How Animation Designer should sample animation. In previous versions there was used 'Mecanim' as default animation sampling mode (it was 'Humanoid IK' toggle)"), mainSetup.Additional_AnimationSampling);
                                ...
                                if( mainSetup.Additional_AnimationSampling == ADClipSettings_Main.EAnimationSamplingMode.Playable && currentMecanim && currentMecanim.applyRootMotion)
                                {
                                    if( mainSetup.ResetPositionOnReplay ) GUI.backgroundColor = Color.green;

                                    if( GUILayout.Button( new GUIContent( " Position", FGUI_Resources.Tex_Refresh, "Setting local position of animator to zero on clip replay - used as temporary solution for playable sampling start position handling on root motion, which can be helpful for maintaining world IK points" ), FGUI_Resources.ButtonStyle, GUILayout.Width(84), GUILayout.Height(18) ) )
                                    {
                                        mainSetup.ResetPositionOnReplay = !mainSetup.ResetPositionOnReplay;
