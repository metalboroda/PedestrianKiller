                ...
                if (Type == EModification.AdditiveRotation)
                {
                    ...
                    if (anglesOff != Vector3.zero)
                    {
                        if (FixedAxisRotation)
                            T.rotation = save.Armature.LatestAnimator.rotation * Quaternion.Euler(anglesOff) * Quaternion.Inverse(save.Armature.LatestAnimator.rotation) * (T.parent.rotation * T.localRotation);
                    ...
                    if (T.childCount > 0)
                    {
                        if (RotationEvaluate2 != null)
                        {
                            ...
                            if (eval2 != 0f) if (RotationValue2 != Vector3.zero)
                                {
                                    if (FixedAxisRotation)
                                        ch.rotation = save.Armature.LatestAnimator.rotation * Quaternion.Euler(RotationValue2 * eval2) * Quaternion.Inverse(save.Armature.LatestAnimator.rotation) * ((ChildRotIndependent ? preTRot : ch.parent.rotation) * ch.localRotation);
