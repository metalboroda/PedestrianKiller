...
Version 1.2.6
- New 'Animation Sampling Mode' property replacing 'Humanoid IK' toggle. It gives ability to use new 'Playable' animation sampling mode.
In the future 'Playable' will become default animation sampling mode since it is faster and cleaner, but first it needs to be verified if all features works properly with this mode.
- Fixed Axis Rotation mode in Bone Rotatation Modifier now don't require character in 0,0,0 rotation
- Few improvements for curve fields handling
- Added 'Bones Pose Fix Offsets' module which needs to be used in the different way than other modules -> like preset, and created with button next to the module 'Remove' button

Version 1.2.5.3.7
- New 'Additive' motion blend options for animation clip Morphs
- Relative space position offset value for IK Align option
