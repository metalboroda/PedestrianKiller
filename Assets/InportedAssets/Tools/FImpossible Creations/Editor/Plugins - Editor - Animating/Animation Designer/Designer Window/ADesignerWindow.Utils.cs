...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public static void DrawCurve(ref AnimationCurve curve, string label = "", int width = 0, float startTime = 0f, float startValue = 0f, float endTime = 1f, float endValue = 1f, float r = 0f, float g = 1f, float b = 1f, float a = 1f, string tooltip = "")
        {
            ...
            Rect rect;
            if (width < 1) rect = EditorGUILayout.GetControlRect();
            else rect = EditorGUILayout.GetControlRect(GUILayout.Width(width));

            Rect curveRange = new Rect(startTime, startValue, endTime - startTime, endValue - startValue);
                        ...
                        case EventType.MouseDown:
                            {
                                if (rect.Contains(e.mousePosition))
                                {
                                    ...
                                    if (e.button == 0)
                                    {
                                        ...
                                        {
                                            AnimationGenerateUtils.LoopCurve(ref curve, false, endTime);
                                            e.Use();
                                        }
                                        else if (e.control)
                                        {
                                    ...
                                    if (e.button == 1)
                                    {
                                        ...
                                        {
                                            float delta = e.shift ? 0.05f : -0.05f;
                                            ...
                                            for (int i = 0; i < curve.keys.Length; i++)
                                            {
                                                ...
                                                if (k.time > endTime)  k.time -= (endTime - startTime);
                                                if (k.time < startTime) k.time += (endTime - startTime);
            ...
            if (width < 1)
            {
                if (string.IsNullOrEmpty(label))
                    curve = EditorGUI.CurveField(rect, curve, new Color(r, g, b, a), curveRange);
                else
                    curve = EditorGUI.CurveField(rect, _curvGC, curve, new Color(r, g, b, a), curveRange);
            }
            else
            {
                if (string.IsNullOrEmpty(label))
            ...
            if (Get)
            {
                Rect clickR2 = new Rect(rect);// GUILayoutUtility.GetLastRect();
                clickR2.position += new Vector2(clickR2.width, 0);
                clickR2.width = 10;
                Get.CurveOptionsButton(ref curve, clickR2, startTime, endTime);

                //Rect buttonRect = new Rect(rect.xMax + 2, rect.y, 16, rect.height);
                //Get.CurveOptionsButton(ref curve, buttonRect, startTime, endTime);
            }
        }
