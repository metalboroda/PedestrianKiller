    ...
    public partial class AnimationDesignerWindow : EditorWindow
    {
        ...
        public void RefreshClipLengthModValues()
        {
            ...
            if (currentMecanim != null) if (currentMecanim.isHuman) if (Anim_MainSet != null)
                        if (Anim_MainSet.AnimationSamplingMode != ADClipSettings_Main.EAnimationSamplingMode.SampleAnimationClip)
                        {
                            skipAnimRange = true;
        ...
        public static bool isCalmingModel = false;
        ...
        public void SampleCurrentAnimation(bool autoElapse = true)
        {
                        ...
                        if( _anim_MainSet.settingsForClip != null )
                        {
                            if( _anim_MainSet.Export_LoopClip != ADClipSettings_Main.ELoopClipDetection.ForceLoop )
                                if( _anim_MainSet.Export_LoopClip == ADClipSettings_Main.ELoopClipDetection.NoLoop || _anim_MainSet.settingsForClip.isLooping == false )
                                {
                                    ...
                                    isCalmingModel = false;
                                }


                            if( _anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.Playable ) // As for now it is more comfortable to reset local position for playable
                                if( _anim_MainSet.ResetPositionOnReplay )
                                    if( currentMecanim && currentMecanim.applyRootMotion )
                                        currentMecanim.transform.localPosition = Vector3.zero;
            ...
            SampleAnimationClip(TargetClip, evlaluatedTime, _play_humanoidPreviewCorrection, _play_humanoidPreviewCorrectionMul);
                ...
                if (latestSecondaryAnimator != null)
                    if (latestSecondaryAnimatorClip != null)
                    {
                        if (latestSecondaryAnimatorClip.length != 0f && TargetClip.length != 0f)
                        {
                            ...
                            if (latestSecondaryAnimatorMecanim == null || latestSecondaryAnimatorMecanim.transform != latestSecondaryAnimator)
                                latestSecondaryAnimatorMecanim = latestSecondaryAnimator.gameObject.GetComponent<Animator>();

                            if (latestSecondaryAnimatorMecanim && _anim_MainSet.AnimationSamplingMode == ADClipSettings_Main.EAnimationSamplingMode.Playable)
                            {
                                UpdatePlayableAnimationPreview(ref playableSecondaryAnimatorPreview, latestSecondaryAnimatorMecanim, latestSecondaryAnimatorClip, secondaryAnimTime);
                            }
                            else
                            {
