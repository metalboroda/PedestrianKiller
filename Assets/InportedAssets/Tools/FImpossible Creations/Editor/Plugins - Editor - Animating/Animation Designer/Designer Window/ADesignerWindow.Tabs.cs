...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerWindow : EditorWindow
    {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (currentMecanim)
                    {
                        ...
                        if (GUILayout.Button(new GUIContent("  Save+", gc_SaveAsClip.image, "Extra export options"), GUILayout.Width(55), GUILayout.Height(22)))
                        {
                            SavePlusGenericMenu(Event.current.button == 1);
        ...
        void SavePlusGenericMenu(bool rmb)
        {
            var saveMenu = new GenericMenu();

            saveMenu.AddItem(new GUIContent("Force exporting all possible animation curves"), false, () => 
            {
                string newClip = SaveClipPopup(rmb);
                if (string.IsNullOrEmpty(newClip) == false) { _exportLegacy = false; AddEditorEvent(() => { ExportToFile(newClip, false, true); }); }
            });

            saveMenu.AddSeparator("");

            // Save and remember directory it's saved in for the next saves
            saveMenu.AddItem(new GUIContent("Save and remember directory"), false, () => 
            {
                string newClip = SaveClipPopup(rmb);
                if (string.IsNullOrEmpty(newClip) == false) { _exportLegacy = false; AddEditorEvent(() => { ExportToFile(newClip, true); }); }
            });
