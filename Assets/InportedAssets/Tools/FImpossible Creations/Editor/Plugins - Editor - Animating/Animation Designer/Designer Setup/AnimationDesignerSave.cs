...
namespace FIMSpace.AnimationTools
{
    public partial class AnimationDesignerSave : ScriptableObject
    {
        ...
        [SerializeField, HideInInspector] int backCompabilityVersionID = 0;
        ...
        private void Reset()
        {
            // Update to current compability version
            backCompabilityVersionID = 1;
        }

        public void OnOpened()
        {
            RefreshCompabilityVersion();
        }

        void RefreshCompabilityVersion()
        {
            if ( backCompabilityVersionID < 1)
            {
                foreach (var clipset in MainSetupsForClips)
                {
                    if (clipset.Additional_UseHumanoidMecanimIK) clipset.Additional_AnimationSampling = ADClipSettings_Main.EAnimationSamplingMode.Mecanim;
                    else clipset.Additional_AnimationSampling = ADClipSettings_Main.EAnimationSamplingMode.SampleAnimationClip;
                }
