...
namespace FIMSpace.AnimationTools
{
    ...
    public class ADClipSettings_Morphing : IADSettings
    {
        ...
        public class MorphingSet : INameAndIndex
        {
            ...
            public enum EBlend
            {
                /// <summary> Default override animation blending </summary>
                Normal,
                /// <summary> Additive local coords apply basing on T-Pose reference </summary>
                Additive,
                /// <summary> Additive local coords apply basing on lastest frame animation pose reference </summary>
                AdditiveDynamic
            }

            public EBlend MotionBlend = EBlend.Normal;
            ...
            private void SampleMorphAnimation(AnimationDesignerSave save)
            {
                ...
                Vector3 preRootPosition = Vector3.zero;
                if (save.LatestAnimator) preRootPosition = save.LatestAnimator.position;

                AnimationDesignerWindow.Get.SampleMorphAnimationClip(MorphWithClip, morphClipTime);
                ...
                {
                    MorphRootPosition = save.LatestAnimator.position;
                    save.LatestAnimator.position = preRootPosition;
                }
            }
            ...
            public void ApplyMorph(float animProgr, AnimationDesignerSave save)
            {
                ...
                {
                    morphLimb.ApplyCapturedLimbState(this, null, blendEval, save);
                }
                ...
                for (int l = 1; l < MorphingLimbSets.Count; l++)
                {
                    ...
                    morphLimb.ApplyCapturedLimbState(this, save.Limbs[morphLimb.LimbIndex], blendEval, save);
                }
            ...
            public class MorphLimbSettings
            {
                ...
                public void ApplyCapturedLimbState(MorphingSet set, ADArmatureLimb limb, float parentBlend, AnimationDesignerSave save)
                {
                    ...
                    if (limb == null)
                    {
                        ...
                        if (set.MotionBlend == EBlend.Normal)
                        {
                        ...
                        else if (set.MotionBlend == EBlend.Additive || set.MotionBlend == EBlend.AdditiveDynamic)
                        {
                            Quaternion referenceRot;
                            Vector3 referencePos = save.ReferencePelvis.localPosition;

                            #region Gathering Reference from TPose 

                            if (set.MotionBlend == EBlend.Additive)
                            {
                                var refPose = save.TPose.GetPoseOf(save.ReferencePelvis.name);
                                if (refPose != null) { referenceRot = refPose.LocalRotation; referencePos = refPose.LocalPosition; }
                                else referenceRot = save.ReferencePelvis.localRotation;
                            }
                            else referenceRot = save.ReferencePelvis.localRotation;

                            #endregion

                            //Vector3 deltaPos = Captured[0].keyLocalPos - referencePos;
                            //save.ReferencePelvis.localPosition = Vector3.LerpUnclamped(save.ReferencePelvis.localPosition, Captured[0].keyLocalPos + deltaPos, blendVal * PositionBlend);
                            save.ReferencePelvis.localPosition = Vector3.LerpUnclamped( save.ReferencePelvis.localPosition, Captured[0].keyLocalPos, blendVal * PositionBlend );

                            Quaternion delta = Quaternion.Inverse(referenceRot) * Captured[0].keyLocalRot;
                            Quaternion additive = Quaternion.SlerpUnclamped(Quaternion.identity, delta, blendVal * RotationBlend);
                            save.ReferencePelvis.localRotation = save.ReferencePelvis.localRotation * additive;
                        }
                    ...
                    else
                    {
                        ...
                        else if ( set.MotionBlend == EBlend.Additive || set.MotionBlend == EBlend.AdditiveDynamic)
                        {
                            for (int l = 0; l < Captured.Count; l++)
                            {
                                ...
                                //Vector3 referencePos = set.MotionBlend == EBlend.AdditiveDynamic ? bone.T.localPosition : bone.InitLocalPos;
                                //Vector3 deltaPos = key.keyLocalPos - referencePos;
                                //bone.T.localPosition = Vector3.LerpUnclamped(bone.T.localPosition, key.keyLocalPos + deltaPos, blendVal * PositionBlend);
                                bone.T.localPosition = Vector3.LerpUnclamped( bone.T.localPosition, key.keyLocalPos, blendVal * PositionBlend );

                                #region Gathering Reference from TPose code backup

                                //Quaternion referenceRot;
                                //if (set.MotionBlend == EBlend.Additive)
                                //{
                                //    var refPose = save.TPose.GetPoseOf(bone.BoneName);
                                //    if (refPose != null) { referenceRot = refPose.LocalRotation; }
                                //    else referenceRot = bone.InitLocalRot;
                                //}
                                //else referenceRot = bone.T.localRotation;

                                #endregion

                                Quaternion delta;
                                if (set.MotionBlend == EBlend.Additive) delta = Quaternion.Inverse(bone.InitLocalRot) * key.keyLocalRot;
                                else delta = key.keyLocalRot * Quaternion.Inverse(bone.T.localRotation);
                                Quaternion additive = Quaternion.SlerpUnclamped(Quaternion.identity, delta, blendVal * RotationBlend);
                                bone.T.localRotation = bone.T.localRotation * additive;
                            }
                        }
                    }
                }
            ...
            internal void DrawParamsGUI(float animProgr, AnimationDesignerSave save)
            {
                ...
                GUILayout.Space(2);
                MotionBlend = (EBlend)EditorGUILayout.EnumPopup("Motion Blending:", MotionBlend);
                GUILayout.Space(4);
