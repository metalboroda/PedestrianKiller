...
namespace FIMSpace.AnimationTools
{
    ...
    public partial class ADClipSettings_CustomModules : IADSettings
    {
        ...
        public static ADClipSettings_CustomModules CopyingFrom = null;
        ...
        public ADClipSettings_CustomModules Copy(ADClipSettings_CustomModules to, AnimationDesignerSave save, bool noCopy)
        {
            ADClipSettings_CustomModules cpy = to;
            if (noCopy == false) cpy = (ADClipSettings_CustomModules)MemberwiseClone();

            cpy.CustomModules = new List<CustomModuleSet>();
            for (int i = 0; i < CustomModules.Count; i++)
            {
                CustomModuleSet nSet = new CustomModuleSet();
                nSet.Enabled = CustomModules[i].Enabled;
                nSet.ModName = CustomModules[i].ModName;
                nSet.Blend = CustomModules[i].Blend;
                nSet.Index = i;
                cpy.CustomModules.Add(nSet);
                CustomModuleSet.PasteValuesTo(CustomModules[i], nSet);
            }
        ...
        public partial class CustomModuleSet : INameAndIndex
        {
            ...
            public static void PasteValuesTo(CustomModuleSet from, CustomModuleSet to)
            {
                ...
                to.ModuleVariables = new List<ADVariable>();
                for (int i = 0; i < from.ModuleVariables.Count; i += 1) to.ModuleVariables.Add(from.ModuleVariables[i].Copy());

                to.TransformsMemory = new List<ADTransformMemory>();
                for (int i = 0; i < from.TransformsMemory.Count; i += 1) to.TransformsMemory.Add(from.TransformsMemory[i].Copy());

                to.ModuleIDHelper = from.ModuleIDHelper;
                to.ModuleReference = from.ModuleReference;
            }
        ...
        internal static void PasteValuesTo(ADClipSettings_CustomModules from, ADClipSettings_CustomModules to)
        {
            for (int i = 0; i < from.CustomModules.Count; i++)
            {
                var cpyModule = from.CustomModules[i];
                to.CustomModules.Add(new CustomModuleSet());
                CustomModuleSet.PasteValuesTo(from.CustomModules[i], to.CustomModules[to.CustomModules.Count - 1]);
            }
        }
